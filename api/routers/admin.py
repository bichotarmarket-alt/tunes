...
async def test_redis_cache(
    ...
    """Testar funcionamento do Redis - escreve e lê uma chave de teste"""
    try:
        test_key = "admin:test:ping"
        test_value = {"timestamp": get_brasilia_time().isoformat(), "user": current_user.email}
        
        # Testar WRITE
        write_ok = await cache.set(test_key, test_value, ttl=60)
        
        # Testar READ
        read_value = await cache.get(test_key)
        
        # Testar TTL
        ttl_remaining = await cache.ttl(test_key)
        
        return {
            "status": "OK" if write_ok and read_value else "FAIL",
            "redis_connected": cache._is_connected,
            "write_success": write_ok,
            "read_success": read_value is not None,
            "value_match": read_value == test_value if read_value else False,
            "ttl_remaining": ttl_remaining,
            "test_data": test_value,
            "message": "✅ Redis funcionando corretamente!" if write_ok and read_value else "❌ Falha no Redis"
        }
    except Exception as e:
        logger.error(f"[ADMIN CACHE] Erro no teste: {e}")
        return {
            "status": "ERROR",
